...
const composeThingAddRoute = thingName => ({
  path: '/:slug?',
  ...
  async action(context, { slug }) {
    ...
    if (!slug && !rbac.can(`${thingName}:add`, { user })) {
    ...
    if (!slug) {
      const addThing = await require.ensure(
        [],
        require => require('./thingAddRoute').default,
        'admin',
      );
      resultRoute = await addThing(thingName, context);
    }
    return resultRoute;
  },
});

export default composeThingAddRoute;
