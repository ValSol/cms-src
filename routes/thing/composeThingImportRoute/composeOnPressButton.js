...
const composeOnPressButton = (thingConfig, context) => async () => {
  ...
  const {
    runtime: { error, excerptsForImport, loading, thingsForImport },
  } = store.getState();
  const { thingName } = thingConfig;

  if (error || loading || !thingsForImport) return;

  const thingsImport = composeThingsImport(thingConfig);
  ...
  try {
    const { data } = await client.mutate({
      mutation: thingsImport,
      variables: {
        excerpts: excerptsForImport,
        things: thingsForImport,
      },
    });
